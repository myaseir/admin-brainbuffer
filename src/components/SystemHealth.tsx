    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
        ...
        <div className="space-y-4">
          <HealthRow label="Database Status" value={health?.database?.connected ? "Operational" : "Offline"} success={health?.database?.connected} />
          <HealthRow label="Total Registered" value={`${health?.database?.total_registered_users} Users`} />
          <HealthRow label="Backend Version" value={health?.system_info?.version} />
          <HealthRow label="Environment" value={health?.system_info?.environment} />
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-xl">
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-end border-b border-slate-800 pb-4">
            ...
            <span className="text-xl font-black">{stats?.gross_collections || 0} PKR</span>
          </div>
          <div className="flex justify-between items-end border-b border-slate-800 pb-4">
            <span className="text-slate-400 text-xs font-bold uppercase">Total Payouts</span>
            <span className="text-xl font-black text-red-400">-{stats?.total_payouts || 0} PKR</span>
          </div>
          <div className="flex justify-between items-end pt-2">
            <span className="text-emerald-500 text-xs font-black uppercase">Net Revenue</span>
            <span className="text-3xl font-black text-emerald-400">{stats?.net_profit || 0} PKR</span>
          </div>
        </div>
      </div>
    </div>
